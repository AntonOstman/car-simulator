...
public:
    ...
    static float improved_perlin_hash(int x, int y, uint32_t seed) {
        ...
        static const glm::vec2 gradients[12] = {
    ...
    static float octave_noise(float x, float y, uint32_t seed, 
                             int octaves = 4, float persistence = 0.5f, float lacunarity = 2.0f) {
        return octave_noise(glm::vec2(x, y), seed, octaves, persistence, lacunarity);
    }

    static uint32_t wang_hash(const glm::ivec2& pos, uint32_t seed) {
        return wang_hash(pos.x, pos.y, seed);
    }

    static float perlin_hash(const glm::ivec2& pos, uint32_t seed) {
        return perlin_hash(pos.x, pos.y, seed);
    }

    static float improved_perlin_hash(const glm::ivec2& pos, uint32_t seed) {
        return improved_perlin_hash(pos.x, pos.y, seed);
    }

    static float perlin_noise_at(float x, float y, uint32_t seed) {
        return perlin_noise_at(glm::vec2(x, y), seed);
    }

    static float perlin_hash_discrete(const glm::ivec2& pos, uint32_t seed) {
        return perlin_hash_discrete(pos.x, pos.y, seed);
    }

    static glm::vec2 improved_perlin_gradient(const glm::ivec2& pos, uint32_t seed) {
        return improved_perlin_gradient(pos.x, pos.y, seed);
    }

    static float terrain_noise(const glm::ivec2& pos, uint32_t seed) {
